...
function App() {
  ...
  useEffect(() => {
    const checkWeatherRisk = (data, locName) => {
      ...
      const isSevere =
        severeConditions.includes(condition) || data.main.temp > 40 || data.main.temp < -5;
  ...
  const handleTrafficUpdate = useCallback((riskLevel) => {
    ...
    setActiveAlerts((prev) => {
      const rest = prev.filter(
        (a) => !a.type.includes("Route Congestion") && !a.type.includes("route congestion")
      );
      if (riskLevel >= 5) {
        return [
          {
            id: "route-congestion-selected",
            type: "Route Congestion Risk",
            severity: riskLevel >= 8 ? "critical" : "warning",
            message: `High risk (Lv ${riskLevel}) on the selected driving route — expect delays or disruptions.`,
            timestamp: "Just now",
            resolved: false,
          },
          ...rest,
